...
cl_int oclGetPlatformID(cl_platform_id* clSelectedPlatformID)
{
    ...
    *clSelectedPlatformID = NULL;
    ...
    {
        Rprintf(" Error %i in clGetPlatformIDs Call !!!\n\n", ciErrNum);
        return -1000;
    }
    else
    {
        if(num_platforms == 0)
        {
            Rprintf("No OpenCL platform found!\n\n");
            return -2000;
        }
        else
        {
            Rprintf("number of OpenCL platform: %d\n", num_platforms);
            // if there's a platform or more, make space for ID's
            if ((clPlatformIDs = (cl_platform_id*)malloc(num_platforms * sizeof(cl_platform_id))) == NULL)
            {
                Rprintf("Failed to allocate memory for cl_platform ID's!\n\n");
                return -3000;
            }
            ...
            for(cl_uint i = 0; i < num_platforms; ++i)
            {
                ...
                if(ciErrNum == CL_SUCCESS)
                {
                    Rprintf("name of OpenCL platform: %s\n", chBuffer);
            ...
            {
                Rprintf("WARNING: NVIDIA OpenCL platform not found - defaulting to first platform!\n\n");
...
//////////////////////////////////////////////////////////////////////////////
//! Gets the device ID of specific type if available, otherwise default
//!
//! @return the id
//! @param clSelectedDeviceID         OpenCL device ID
//////////////////////////////////////////////////////////////////////////////
cl_int oclGetDeviceID(cl_device_id* clSelectedDeviceID, cl_device_type type)
{
    cl_uint num_platforms;
    cl_platform_id* clPlatformIDs;
    cl_int ciErrNum;
    *clSelectedDeviceID = NULL;
...
//            Rprintf("number of OpenCL platform: %d\n", num_platforms);
            ...
            // get platform info for each platform and get the device of type if found
            ciErrNum = clGetPlatformIDs (num_platforms, clPlatformIDs, NULL);
            for(cl_uint i = 0; i < num_platforms; ++i)
            {
                cl_uint num_device;
                ciErrNum = clGetDeviceIDs(clPlatformIDs[i], type, 0,NULL,&num_device);
                ciErrNum = clGetDeviceIDs(clPlatformIDs[i], type, 1, clSelectedDeviceID, NULL);
                if(ciErrNum == CL_SUCCESS)
                {
                    // CL_DEVICE_NAME
                    char device_string[1024];
                    clGetDeviceInfo(*clSelectedDeviceID, CL_DEVICE_NAME, sizeof(device_string), &device_string, NULL);
                    Rprintf ("OpenCL Device: %s\n", device_string);
                    break;
                }
            }

            // device not found
            if(*clSelectedDeviceID == NULL)
            {
                Rprintf("ERROR: device not found!\n\n");
                free(clPlatformIDs);
                return -4000;
            }
